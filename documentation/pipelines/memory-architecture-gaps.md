...
> **Статус (2026-07-11):** инженерный hotfix L3 выполнен как **Stage 9.1** (bounded `expand_graph_context`, interactive timeout, TransientError soft-fail, E2E `--no-llm`/`--skip-neo4j`). **Этап 1** (hybrid rerank) внедрён в [`backend/services/rerank.py`](../../backend/services/rerank.py) и подключён в search/E2E.  
> **Этапы 2 и 3 явно отложены** до стабильного Precision@5 и контролируемой L3 latency — не начинать Event/Turn Mode Controller и Dynamic Working Memory / multi-step $Q^{(t)}$, пока метрики и бюджет Neo4j не зафиксированы.

### Этап 1: Оптимизация контекста и умный Rerank (MVP — Краткосрочный приоритет) — *in progress / landed core*
* **Цель:** Повысить Precision@5 с 0.15 до 0.40 и устранить замусоривание L4-контекста.
* **Задачи:**
  1. ~~Реализовать гибридную формулу скоринга (Rule-based Reranker)~~ — `hybrid_rerank` + `graph_support` из bounded expand.
...
### Этап 2: Иерархический роутинг (Среднесрочный приоритет) — **DEFERRED**
* **Статус:** отложен до стабильного Precision@5 и L3 (после доводки Этапа 1: Structured Fact Extraction + No-Evidence Path).
...
### Этап 3: Динамическая эволюция рабочей памяти (Долгосрочный приоритет) — **DEFERRED**
* **Статус:** отложен; не начинать из-за медленного/нестабильного Neo4j или до Этапа 2.
